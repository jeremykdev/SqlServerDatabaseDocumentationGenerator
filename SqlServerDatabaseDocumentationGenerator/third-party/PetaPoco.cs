...
namespace PetaPoco
{
	...
	// For explicit pocos, marks property as a indexList column and optionally supplies column name
	[AttributeUsage(AttributeTargets.Property)]
	...
	public class Database : IDisposable
	{
		...
		public void BuildPageQueries<T>(long skip, long take, string sql, ref object[] args, out string sqlCount, out string sqlPage) 
		{
			...
			// Build the SQL for the actual final indexList
		...
		// Find the split point in a indexList set for two different pocos and return the poco factory for the first
			...
			try
			{
				...
				try
				{
					using (var cmd = CreateCommand(_sharedConnection, ""))
					{
						...
						{
							// Don't insert indexList columns
					...
					using (var cmd = CreateCommand(_sharedConnection, ""))
					{
						...
						if (columns == null)
						{
							foreach (var i in pd.Columns)
							{
								...
								// Dont update indexList only columns
								if (i.Value.ResultColumn)
									continue;
